...
for (m in seq_along (jr_var)) {
	...
	# Code using fastai, calling python from R
...
saveRDS(list (pred_list_walk2 = pred_list_walk2,
        ...
        "output/res_sametasks.RDS")

```

```{r}

res2 <- readRDS("output/res_sametasks.RDS")
...
error_df <- bind_rows(error_walk,
  ...
  mutate (epochs = factor (epochs, levels = c("25", "50", "100", "200", "500", "1000")),
         type = factor (type, levels = c("walk", "run"))) %>%
...
tiff ("manuscript/fig5_same_perf.tiff", units = "in", height = 7, width = 17, res = 100)
...
tiff ("manuscript/fig2_same_curv.tiff", units = "in", height = 15, width = 10, res = 100)
...
writexl::write_xlsx (list( Same = export_df),
                     "manuscript/supp_mat_same.xlsx")
```
